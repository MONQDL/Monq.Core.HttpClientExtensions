...
{
    static readonly CamelCasePropertyNamesContractResolver _jsonResolver = new() { NamingStrategy = new CamelCaseNamingStrategy { ProcessDictionaryKeys = true } };
    ...
    readonly RestHttpClientOptions _configuration = new();
    ...
    public RestHttpClientTests()
    {
        ...
        {
            ForwardedHeaders = [TraceEventIdHeader, UserspaceIdHeader]
        });
    }
    ...
    public async Task ShouldProperlyMakeGetRequestByAbsoluteUri()
    {
        ...
        {
            new() { Id = 1, Name = "Service1" },
            new() { Id = 2, Name = "Service2" }
        });
    ...
    public async Task ShouldProperlyMakeGetRequestByAbsoluteUriWithForwardedHeaders()
    {
        ...
        httpContext.Request.Headers.Append(TraceEventIdHeader, traceEventId);
        httpContext.Request.Headers.Append(UserspaceIdHeader, userspaceId);
    ...
    public async Task ShouldProperlyReuseHttpClientObjectInParallelWithHeaders()
    {
        ...
        var httpContext = new DefaultHttpContext();
        httpContext.Request.Headers.Append(TraceEventIdHeader, "eventId");
        httpContext.Request.Headers.Append(UserspaceIdHeader, "12");
        ...
        var results = Enumerable.Range(0, totalRequests).AsParallel().AsOrdered().WithDegreeOfParallelism(10)
            ...
            .AsSequential()
            .ToList();
        Assert.DoesNotContain(results, x => x.OriginalResponse?.RequestMessage?.Headers.Count() != 2);
    ...
    {
        var httpContext = new DefaultHttpContext();
        httpContext.Request.Headers.Append("Authorization", "Bearer token355");

        var client = new HttpClient(CreateDefaultResponseHandler(HttpStatusCode.Unauthorized, "{}"));
        _ = CreateRestHttpClient(client, httpContext);
    ...
    {
        var httpContext = new DefaultHttpContext();
        httpContext.Request.Headers.Append("Authorization", "Bearetoken355");
    ...
    RestHttpClientMock CreateRestHttpClient(
    ...
    static HttpMessageHandler CreateDefaultResponseHandler(HttpStatusCode responseCode, string jsonResponseBody)
    {
        var mockHttpMessageHandler = new Mock<HttpMessageHandler>();
